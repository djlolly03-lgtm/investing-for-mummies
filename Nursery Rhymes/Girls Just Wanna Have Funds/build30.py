...
A=json.load(open('anchors30.json')); FPS=30
CROP={'02':'crop=556:988:82:0'}
OUT='w30'; os.makedirs(OUT,exist_ok=True)
...
for k in ['W1','W2','W3','W4','W5','W6']:
    d=A[k]; clip=d['clip']; c=d['c']; t=d['t']          # t is already VIDEO time
...
for i,(k,clip,a,b,s,e) in enumerate(parts):
    nf=int(round(e*FPS))-int(round(s*FPS)); want+=nf      # absolute video time -> no drift
    base=(e-s)/(b-a); pad=0.0; bo=1.0
    for _ in range(7):
        r=base*bo
        key=hashlib.md5(f'{clip}|{a:.4f}|{b:.4f}|{e-s:.4f}|{nf}|{pad:.3f}|{bo:.4f}|{CROP.get(clip,"")}'.encode()).hexdigest()[:10]
        ...
        if not (os.path.exists(out) and os.path.getsize(out)>1000):
            ...
            subprocess.run(['ffmpeg','-v','error','-y','-i',f'clip-{clip}.mp4','-filter_complex',
              f"[0:v]{','.join(vf)}[v]",'-map','[v]','-frames:v',str(nf),'-an','-c:v','libx264',
              '-crf','16','-preset','medium','-pix_fmt','yuv420p',out],check=True)
        n=nfr(out)
        if n>=nf: break
    ...
    if n<nf: print(f"  p{i:03d} {k} SHORT {n}/{nf}")
print(f"frames want {want} got {got}")
...
print('body =',subprocess.run(['ffprobe','-v','error','-show_entries','format=duration','-of','csv=p=0',
      f'{OUT}/body.mp4'],capture_output=True,text=True).stdout.strip(),'expected 28.24')
