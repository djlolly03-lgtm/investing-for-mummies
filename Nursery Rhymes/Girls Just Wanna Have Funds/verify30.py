...
A=json.load(open('anchors30.json')); song=json.load(open('song-words-v2.json'))
...
for k in ['W1','W2','W3','W4','W5','W6']:
    ...
    if clip=='10': print(f"  {k} clip-10: IFM logo reveal, no mouth on screen"); continue
...
subprocess.run(['ffmpeg','-v','error','-y','-i','girls-just-wanna-have-funds.mp4','-vf',
  'scale=160:284,tblend=all_mode=difference','-pix_fmt','gray','-f','rawvideo','diag/fd30.raw'],check=True)
a=np.fromfile('diag/fd30.raw',dtype=np.uint8); H,W=284,160
