...
END=29.48
w=wave.open('song5-st.wav'); sr=w.getframerate()
...
LINES=[(0.00,3.28,"Smart with money, still have fun"),
 (3.82,8.02,"That's Investing for Mummies"),
 (11.28,14.02,"Girls just wanna have fun"),
 (15.28,17.92,"Girls just wanna have fun"),
 (19.20,22.84,"Have your fun, enjoy the ride"),
 (23.40,27.40,"Keep your future on your side")]
CAP={1:0.90}   # "That is Investing for Mummies" - vocal really ends ~0.9s after the last word
CAPS=[]
for i,(s,lw,txt) in enumerate(LINES):
    nxt=LINES[i+1][0] if i+1<len(LINES) else END
    e=min(vend(lw,min(nxt-0.12,END)),nxt-0.12)
    e=min(e, lw+CAP.get(i,1.6))       # hard cap: don't let centred instrumental extend a caption
    CAPS.append((s,e,txt))
...
os.makedirs('caps5',exist_ok=True)
def fit(t):
...
for i,(s,e,txt) in enumerate(CAPS):
    ...
    p=f'caps5/cap{i:02d}.png'; im.save(p)
    meta.append({'file':p,'start':round(s,3),'end':round(e,3),'h':H,'text':txt})
json.dump(meta,open('caps5/meta.json','w'),indent=1)
