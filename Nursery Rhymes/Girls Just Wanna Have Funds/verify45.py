...
A=json.load(open('anchors45.json')); song=json.load(open('song-words-v2.json'))
...
subprocess.run(['ffmpeg','-v','error','-y','-i','girls-just-wanna-have-funds.mp4','-vf',
  'scale=160:284,tblend=all_mode=difference','-pix_fmt','gray','-f','rawvideo','diag/fd45.raw'],check=True)
a=np.fromfile('diag/fd45.raw',dtype=np.uint8); H,W=284,160
