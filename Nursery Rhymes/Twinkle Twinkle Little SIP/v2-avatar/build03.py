#!/usr/bin/env python3
"""Twinkle Twinkle Little SIP - v2 (AI avatar clips) - build 03
  ...
  fix: every cut pinned to an exact 30fps frame index so no cumulative drift.
...
for name, src, anch in PLAN:
    ...
    dst = f"{WORK}/{name}.mp4"
    nframes = round((anch[-1][1] - anch[0][1]) * FPS)
    run(["ffmpeg", "-y", "-v", "error", "-i", f"{SRC}/{src}.mp4",
         "-filter_complex", chain, "-map", "[v]", "-an", "-frames:v", str(nframes),
...
run(["ffmpeg", "-y", "-v", "error",
     ...
     f"{OUT}/twinkle-sip-v2-build03.mp4"])
print("\n->", f"{OUT}/twinkle-sip-v2-build03.mp4")
