#!/usr/bin/env python3
"""Twinkle Twinkle Little SIP - v2 build 07.  Full playbook pass.
...
GEOM = {"C-coins-jar": f"crop=668:1188:26:90,scale={W}:{H}",
        "LOGO-golden-light": None}   # None -> LOGO_GEOM (landscape pad + breathe)
...
SRCDUR = {}
for _b in BLOCKS:
    if _b["src"] not in SRCDUR:
        SRCDUR[_b["src"]] = float(subprocess.run(["ffprobe","-v","error",
            "-show_entries","format=duration","-of","csv=p=0",
            f"{SRC}/{_b['src']}.mp4"], capture_output=True, text=True).stdout)
...
for i, b in enumerate(BLOCKS):
    ...
    nf_slot = round(slot[1] * FPS) - round(slot[0] * FPS)
    xf_f = round(nxt_xf * FPS)
    nf = nf_slot + xf_f
    want = nf_slot / FPS          # the solve targets the REAL slot only; the
                                  # dissolve tail is appended afterwards so it
                                  # cannot distort the lip anchors
    ...
    if xf_f:
        c0, e0, rr0 = segs[-1]
        tail = (xf_f / FPS) / rr0
        segs = segs + [[e0, min(e0 + tail, SRCDUR.get(src, 10.0)), rr0]]
    # setpts retiming lands a frame or two short of the requested count, and
    # -frames:v cannot invent frames (cloning is banned by playbook 6).  So run
    # the last segment a little past its end; the exact -frames:v count trims it
    # back, leaving timing untouched.
    c0, e0, rr0 = segs[-1]
    segs[-1] = [c0, min(e0 + 0.30, SRCDUR.get(src, 10.0)), rr0]
    parts = [f"[0:v]trim={c:.4f}:{e:.4f},setpts=(PTS-STARTPTS)*{rr:.6f}[p{j}]"
             for j, (c, e, rr) in enumerate(segs)]
    n = len(parts)
    geom = GEOM.get(src, f"scale={W}:{H}") or LOGO_GEOM
...
run(["ffmpeg", "-y", "-v", "error", "-i", acc, "-i", SONG,
     ...
     f"{OUT}/twinkle-sip-v2-build07.mp4"])
av = dur(acc)
print(f"\nassembled video: {av:.3f}s   song: 45.840s   delta: {av-45.84:+.3f}s")
print("->", f"{OUT}/twinkle-sip-v2-build07.mp4")
