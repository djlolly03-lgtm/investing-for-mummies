#!/usr/bin/env python3
"""Twinkle Twinkle Little SIP - v2 (AI avatar clips) - build 01
...
PLAN = [
    # name, source, anchors
    ("01-logo", "LOGO-golden-light", [(3.62, 0.00), (6.30, 2.68)]),
...
run(["ffmpeg", "-y", "-v", "error",
     ...
     "-filter_complex",
     f"[0:v][1:v]xfade=transition=fade:duration={XF}:offset={2.68-XF:.3f},format=yuv420p[v]",
     "-map", "[v]", "-map", "2:a",
     "-c:v", "libx264", "-preset", "slow", "-crf", "18", "-pix_fmt", "yuv420p",
     "-c:a", "aac", "-b:a", "192k", "-shortest",
     f"{OUT}/twinkle-sip-v2-build01.mp4"])
print("\n->", f"{OUT}/twinkle-sip-v2-build01.mp4")
